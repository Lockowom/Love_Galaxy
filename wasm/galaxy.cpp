...
// ------------------------------------------------------------------ constantes
enum { T_HEART = 0, T_MSG = 1, T_ROCK = 2, T_POWER = 3 };
...
static float shieldT;      // segundos restantes de escudo (power-up)
static int   lastPickup;   // 1 si en este frame se recogió corazón/mensaje
static int   lastMsg;      // 1 si en este frame se recogió un MENSAJE (para frase)
static int   lastHit;      // 1 si en este frame chocó con una roca
...
static void spawn() {
    ...
    float roll = frand();
    if (roll < 0.50f) {                 // corazón (común)
        e.type = T_HEART;
        e.r    = minDim * 0.032f;
        e.vx   = base * rrange(0.9f, 1.1f);
        e.wob  = minDim * 0.05f;
    } else if (roll < 0.66f) {          // mensaje de amor (puntúa más)
        e.type = T_MSG;
        e.r    = minDim * 0.040f;
        e.vx   = base * rrange(0.8f, 1.0f);
        e.wob  = minDim * 0.07f;
    } else if (roll < 0.94f) {          // roca / asteroide (peligro)
        e.type = T_ROCK;
        e.r    = minDim * rrange(0.038f, 0.060f);
        e.vx   = base * rrange(1.0f, 1.35f);
    } else {                            // power-up escudo (raro)
        e.type = T_POWER;
        e.r    = minDim * 0.036f;
        e.vx   = base * rrange(0.85f, 1.0f);
        e.wob  = minDim * 0.04f;
    }
}
...
void resize(float w, float h) {
    // Mantener posiciones relativas al cambiar el tamaño del lienzo.
...
int update(float dt) {
    ...
    if (state == 1) {
        ...
        // Movimiento suave de la nave hacia el puntero
        ...
        // Mover, oscilar, colisiones, limpieza
        for (int i = 0; i < MAX_ENT; i++) {
            ...
            if (circlesHit(playerX, playerY, playerR, e.x, drawY, e.r)) {
                if (e.type == T_HEART) { score += 10; lastPickup = 1; e.active = 0; continue; }
                if (e.type == T_MSG)   { score += 30; lastPickup = 1; lastMsg = 1; e.active = 0; continue; }
                if (e.type == T_POWER) { shieldT = 6.0f; lastPickup = 1; e.active = 0; continue; }
                if (e.type == T_ROCK) {
                    if (shieldT > 0.0f) { shieldT = 0.0f; e.active = 0; continue; } // escudo absorbe
                    lives--; lastHit = 1; e.active = 0;
...
// Buffer de dibujo: puntero (offset en la memoria lineal) y nº de entidades.
